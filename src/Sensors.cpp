...
void Sensors::loop()
{
    ...
    if ((millis() - pmLoopTimeStamp > sample_time * (uint32_t)1000))
    { // sample time for each capture
        pmLoopTimeStamp = millis();
        readAllSensors();

        if (!dataReady)
            DEBUG("-->[SLIB] any data from sensors\t: ? check your wirings!");
...
void Sensors::setSampleTime(int seconds)
{
    ...
    if (isSensorRegistered(SENSORS::SSCD30))
    {
        ...
        if (devmode)
            Serial.println("-->[SLIB] SCD30 interval time\t: " + String(seconds));
    }
}
...
void Sensors::setCO2RecalibrationFactor(int ppmValue)
{
    ...
    if (isSensorRegistered(SENSORS::SSCD4X))
    {
        ...
        if (error)
            Serial.printf("-->[SLIB] SCD4X recalibration\t: error frc:%d\r\n", frcCorrection);
        delay(50);
...
bool Sensors::isSensorRegistered(SENSORS sensor)
{
    ...
    {
        if (sensors_registered[i] == sensor)
            return true;
    }
    return false;
}
...
{
    if (sensor < 0 || sensor > SENSORS::SCOUNT)
        return "";
...
{
    if (!isSensorRegistered(SENSORS::SSPS30))
        return false;
...
void Sensors::bme280Read()
{
    ...
    if (isnan(humi1) || humi1 == 0 || isnan(temp1))
        return;
    humi = humi1;
    temp = temp1 - toffset;
...
void Sensors::bmp280Read()
{
    ...
    if (press1 == 0 || isnan(temp1) || isnan(alt1))
        return;
    temp = temp1 - toffset;
...
bool Sensors::sensorSerialInit(u_int pms_type, int pms_rx, int pms_tx)
{
    ...
    if (pms_type == SENSORS::Auto)
    {
        ...
        if (!serialInit(pms_type, 9600, pms_rx, pms_tx))
            return false;
    }
    ...
    else if (pms_type == SENSORS::SSPS30)
    {
        ...
        if (!serialInit(pms_type, 115200, pms_rx, pms_tx))
            return false;
    }
    ...
    {
        DEBUG("-->[SLIB] UART detecting type\t: SENSEAIRS8");
        if (!serialInit(pms_type, PM1006::BIT_RATE, pms_rx, pms_tx))
            return false;
    }
...
{
    if (dev_uart_type == SENSORS::SSPS30)
        return false;
    sensorAnnounce(SENSORS::SSPS30);
    // set driver debug level
    if (CORE_DEBUG_LEVEL > 0)
...
void Sensors::bme280Init()
{
    ...
    if (!bme280.begin() &&
        ...
        !bme280.begin(BME280_ADDRESS_ALTERNATE, &Wire1))
        return;
#endif
...
void Sensors::aht10Init()
{
    ...
    if (!aht10.begin(EXT_I2C_SDA, EXT_I2C_SCL, 100000, 50000))
        return;
#else
    if (!aht10.begin())
        return;
...
void Sensors::CO2scd30Init()
{
    ...
    if (!scd30.begin())
        return;
#else
    if (!scd30.begin() && !scd30.begin(SCD30_I2CADDR_DEFAULT, &Wire1, SCD30_CHIP_ID))
        return;
#endif
    delay(10);
...
void Sensors::GCJA5Init()
{
    ...
    if (!pmGCJA5.begin())
        return;
#else
    if (!pmGCJA5.begin() && !pmGCJA5.begin(Wire1))
        return;
#endif
...
void Sensors::enableGeigerSensor(int gpio)
{
    ...
    {
        if (devmode)
            Serial.printf("[W][SLIB] undefined Geiger pin\t: %i\r\n", gpio);
        return;
    }
